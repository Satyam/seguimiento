// Modified to use Bootstrap Glyphicons instead of Font Awesome
L.Control.SlideMenu = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        link.title = 'Menu';
        L.DomUtil.create('span', 'glyphicon glyphicon-menu-hamburger', link);

        this._menu = L.DomUtil.create('div', 'leaflet-menu', map._container);
        this._menu.style.width = this.options.width;
        this._menu.style.height = this.options.height;

        var closeButton = L.DomUtil.create('button', 'leaflet-menu-close-button glyphicon', this._menu);

        if (this._isLeftPosition) {
            this._menu.style.left = '-' + this.options.width;
            closeButton.style.float = 'right';
            L.DomUtil.addClass(closeButton, 'glyphicon-fast-backward');
        }
        else {
            this._menu.style.right = '-' + this.options.width;
            closeButton.style.float = 'left';
            L.DomUtil.addClass(closeButton, 'glyphicon-fast-forward');
        }
